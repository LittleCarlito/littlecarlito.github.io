...
if [ "$1" == "--help" ] || [ "$1" == "-h" ]; then
  ...
  echo "  --auto-changeset-prefix <prefix>  Prefix for auto-generated changeset commits to exclude (default: auto-)"
  echo ""
  exit 0
fi
...
if [[ $BASE_COMMIT =~ ^[0-9a-f]{7,40}$ ]]; then
  ...
  CONVENTIONAL_COUNT=$(git log $BASE_COMMIT..HEAD --format=%s | grep -E '^(feat|fix|chore|docs|style|refactor|perf|test|build|ci|revert)(\([^)]+\))?!?:' | grep -v "$AUTO_CHANGESET_PREFIX-generate changeset" | wc -l | tr -d ' ')
  RANGE="$BASE_COMMIT..HEAD"
else
  # If it's a date or other reference
  CONVENTIONAL_COUNT=$(git log --since="$BASE_COMMIT" --format=%s | grep -E '^(feat|fix|chore|docs|style|refactor|perf|test|build|ci|revert)(\([^)]+\))?!?:' | grep -v "$AUTO_CHANGESET_PREFIX-generate changeset" | wc -l | tr -d ' ')
