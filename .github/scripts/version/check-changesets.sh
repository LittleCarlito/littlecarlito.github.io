...
check_changesets() {
    ...
    # Check for any changesets (excluding README.md and config.json)
    CHANGESET_COUNT=$(find .changeset -type f -name "*.md" ! -name "README.md" | wc -l | tr -d ' ')
    ...
    if [ -n "$auto_changeset_prefix" ]; then
        AUTO_CHANGESET_COUNT=$(find .changeset -type f -name "${auto_changeset_prefix}*.md" | wc -l | tr -d ' ')
