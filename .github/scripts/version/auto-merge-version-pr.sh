...
echo "Searching for version PRs with title: '$PR_TITLE'"
...
if [[ -z "$MATCHING_PR" || "$MATCHING_PR" == "null" ]]; then
  echo "No open PRs found with title: '$PR_TITLE'"
...
if [[ -n "$PR_NUMBER" && "$PR_NUMBER" != "null" ]]; then
  echo "Found matching PR #$PR_NUMBER"
  ...
  echo "PR #$PR_NUMBER mergeable: $MERGEABLE, state: $MERGEABLE_STATE"
  
  # Auto-merge the PR if it's mergeable
  if [[ "$MERGEABLE" == "true" || "$MERGEABLE_STATE" == "clean" ]]; then
    echo "Auto-merging PR #$PR_NUMBER using $MERGE_METHOD method"
    ...
    if [[ "$MERGED" == "true" ]]; then
      ...
      if [[ "$DELETE_BRANCH" == "true" && -n "$PR_HEAD" ]]; then
        echo "Deleting branch: $PR_HEAD"
        curl -s -X DELETE -H "$HEADER_AUTH" -H "$HEADER_ACCEPT" \
          "https://api.github.com/repos/$REPO/git/refs/heads/$PR_HEAD"
        echo "branch_deleted=true" >> $GITHUB_OUTPUT
      fi
    else
      echo "Failed to merge PR #$PR_NUMBER"
      echo "Error message: $(echo "$MERGE_RESPONSE" | jq -r '.message')"
      echo "pr_merged=false" >> $GITHUB_OUTPUT
    fi
  else
    echo "PR #$PR_NUMBER is not mergeable at this time (state: $MERGEABLE_STATE)"
    echo "pr_merged=false" >> $GITHUB_OUTPUT
  fi
else
  echo "No valid PR number found in the response"
  echo "has_pr=false" >> $GITHUB_OUTPUT
fi
