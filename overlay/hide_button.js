...
export class HideButton {
    ...
    constructor(incoming_scene, incoming_camera) {
        ...
        this.hide_button.position.y = this.get_hide_button_y(incoming_camera);
        this.hide_button.position.x = this.get_hide_button_x(true, incoming_camera);
    ...
    get_hide_button_x(is_column_left, incoming_camera) {
        return is_column_left ? (get_screen_size(incoming_camera).x / 2) - 2.5 : get_associated_position(EAST, incoming_camera);
    }
    
    /** Calculates hide button y position based off camera position and window size */
    get_hide_button_y(incoming_camera) {
        return (get_screen_size(incoming_camera).y / 2) - 2.5;
    }
    ...
    swap_sides(is_column_left, incoming_camera) {
        const hide_x = this.get_hide_button_x(is_column_left, incoming_camera);
    ...
    reposition(is_column_left, incoming_camera) {
        new Tween(this.hide_button.position)
        .to({ 
            x: this.get_hide_button_x(is_column_left, incoming_camera),
            y: this.get_hide_button_y(incoming_camera)
        })
