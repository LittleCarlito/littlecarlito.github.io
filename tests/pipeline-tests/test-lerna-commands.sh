...
pnpm lerna changed || echo "No changed packages detected"

# Try to get help for diff command
echo "Diff command help:"
pnpm lerna diff --help || echo "Help command failed"

# Try diff command with a package (if any exists)
FIRST_PACKAGE=$(pnpm lerna list --json | jq -r '.[0].name' 2>/dev/null || echo "")
if [ -n "$FIRST_PACKAGE" ]; then
  echo "Testing diff on package: $FIRST_PACKAGE"
  pnpm lerna diff "$FIRST_PACKAGE" || echo "Diff command failed"
fi
